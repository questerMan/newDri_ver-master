...
-(MoreMenuView *)moreMenu{
    if (_moreMenu == nil) {
        _moreMenu = [[MoreMenuView alloc] init];
        _moreMenu.autoresizingMask = UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight;
        
        [_moreMenu setDelegate:self];
        
        [_moreMenu setTrackingMode:self.driveView.trackingMode];
        [_moreMenu setShowNightType:self.driveView.showStandardNightType];
...
{
    //进行路径规划
    [self.driveManager calculateDriveRouteWithEndPoints:@[self.endPoint] wayPoints:@[self.endPoint] drivingStrategy:AMapNaviDrivingStrategySingleDefault];
}
